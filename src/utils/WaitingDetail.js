import Waiting0 from "../assets/WaitingImage/Waiting0.jpeg";
import Waiting1 from "../assets/WaitingImage/Waiting1.jpeg";
import Waiting2 from "../assets/WaitingImage/Waiting2.jpeg";
import Waiting3 from "../assets/WaitingImage/Waiting3.jpeg";
import Waiting4 from "../assets/WaitingImage/Waiting4.jpeg";
import Waiting5 from "../assets/WaitingImage/Waiting5.jpeg";
import Waiting6 from "../assets/WaitingImage/Waiting6.jpeg";
import Waiting7 from "../assets/WaitingImage/Waiting7.jpeg";

export const WaitingImage = [{ id: "0", name: "Waiting1", src: Waiting1 }];
export const contents = [
  {
    title: "폭우로 인해 집과 가족을 잃은 이들",
    organization: "서울시사회복지센터",
    image: Waiting0,
    progress: 83,
    barWidth: "210px",
    total: "10,209,000",
    target: "12,300,000",
    categories: ["all", "disaster"],
    date: "2023년",
    text: "물과 나무, 구름과 바람이 어우러진 곳에서 시작된 작은 눈송이는 자연의 미학을 담아낸다. 땅 위에 머물며 시간의 흐름을 탐구하며, 간직한 기억들을 부드러운 날개에 실어 먼 곳으로 날아간다. 아름다운 풍경과 더불어 마음 속 깊은 곳에서 소중한 추억들이 새록새록 녹아든다. 멈추지 않고 계속되는 세월의 흐름 속에서도 작은 눈송이는 우아하게 춤을 추며 시간을 읊조린다. 사람들의 마음을 따뜻하게 만드는 작은 눈송이는 모두에게 소중한 선물이다. 그 소박한 아름다움은 언제나 기억 속에 머물러 영원히 빛나고 있을 것이다.",
  },
  {
    title: "이 아이가 배부름을 알까요",
    organization: "세이브더 칠드런",
    image: Waiting1,
    progress: 52,
    barWidth: "140px",
    total: "6,396,000",
    target: "12,300,000",
    categories: ["all", "weak"],
    date: "2023년",
  },
  {
    title: "홍수로 집을 잃은 사람들",
    organization: "강서구청",
    image: Waiting2,
    progress: 89,
    barWidth: "220px",
    total: "10,947,000",
    target: "12,300,000",
    categories: ["all", "disaster"],
    date: "2023년",
  },
  {
    title: "지영이, 혼자가 아니야!",
    organization: "굿네이버스",
    image: Waiting3,
    progress: 42,
    barWidth: "120px",
    total: "5,166,000",
    target: "12,300,000",
    categories: ["all", "weak"],
    date: "2023년",
    text: `지영이는 한 부모 가정에서 동생을 돌보면서 혼자 지내고 있어요.
                어린 나이에도 불구하고 엄청난 책임감으로 동생을 보살피며
                자라나고 있어요. 하지만 어려운 환경과 경제적 어려움으로 인해
                많은 어려움을 겪고 있어요. 그런데 지영이는 불평하지 않아요.
                오히려 어려운 상황에서도 밝은 미소를 지어내며 살아가고 있어요.
                그런 지영이를 보면 마음이 따뜻해지고, 더 많은 사람들이 그녀를
                도와주고 싶어지는 거예요. 우리의 작은 후원이라도 지영이에게는 큰
                힘이 될 거예요. 그녀의 꿈을 이룰 수 있도록 함께 도와주세요. 지금
                지영이와 함께한 기부로 행복을 나누어주세요. 그녀의 미래를 밝고
                희망찬 것으로 만들어주기 위해 지금 바로 기부해주세요! 당신의
                작은 선물이 지영이에게는 큰 응원이 될 거예요. 지영이가 더 나은
                미래를 만들 수 있도록 우리의 손길을 건네주세요. 함께하는
                여러분의 따뜻한 마음이 그녀에게 힘이 될 거예요. 지금 당신의 손을
                내밀어 지영이와 함께 희망의 문을 열어주세요! 지영이는 훌륭한
                아이에요. 그녀의 잠재력을 발견하고 응원하는 것은 우리 모두에게
                큰 보람이 될 거예요. 그녀의 꿈과 희망을 응원하고 지원하기 위해
                지금 바로 행동해주세요. 지영이를 돕기 위해 우리 모두의 손길을
                모아주세요! 감사합니다. 지영이와 함께하는 따뜻한 마음을
                전달해주셔서요. 지영이가 더 나은 미래를 만들 수 있도록 우리의
                성원을 보내주세요. 함께해주셔서 감사합니다!`,
  },
  {
    title: "강원 산불 피해 모금",
    organization: "한국재난관리협회",
    image: Waiting4,
    progress: 35,
    barWidth: "100px",
    total: "4,350,000",
    target: "12,300,000",
    categories: ["all", "disaster", "environment"],
    date: "2023년",
  },
  {
    title: "소아암 환자들을 위한 의료비 지원",
    organization: "사랑의열매",
    image: Waiting5,
    progress: 21,
    barWidth: "73px",
    total: "2,583,000",
    target: "12,300,000",
    categories: ["all", "weak", "medical"],
    date: "2023년",
  },
  {
    title: "이도윤을 위한 급식비",
    organization: "울산제일고등학교",
    image: Waiting6,
    progress: 99,
    barWidth: "245px",
    total: "12,280,000",
    target: "12,300,000",
    categories: ["all", "weak"],
    date: "2023년",
  },
  {
    title: "2plusone이 물에 잠겼어요",
    organization: "동국대학교",
    image: Waiting7,
    progress: 40,
    barWidth: "110px",
    total: "5,583,000",
    target: "12,300,000",
    categories: ["all", "disaster"],
    date: "2023년",
  },
  // 다른 컨텐츠들도 추가...
];
